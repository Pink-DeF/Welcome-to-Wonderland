#include "../MasterOfPuppet.h"

void MasterOfPuppet::getMovePosition()
{
...
void MasterOfPuppet::useFake()
{
    _data->MasterFakePosition = static_cast<size_t>(rand() % 30);
}

MasterOfPuppet::MasterOfPuppet(std::shared_ptr<nightDB> data, size_t ID)
{
    _ID = ID;
    _data = data;
...
bool MasterOfPuppet::move(bool doorStatus)
{
    if(_enemyActive)
    {
